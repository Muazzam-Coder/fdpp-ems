...
#                 # FIX: Look inside 'data' instead of 'record'
#                 inner_data = data.get('data', {})
#                 emp_name = inner_data.get('employee_name', 'Unknown')
#                 is_late = inner_data.get('is_late', False)
#                 late_msg = inner_data.get('late_message', '')
...
class BiometricMonitor:
    def __init__(self, device_ip, device_port, server_url):
        self.device_ip = device_ip
        self.device_port = device_port
        self.server_url = server_url
        self.conn = None
    ...
    def connect_device(self):
        try:
            ...
            self.conn = zk.connect()
            # Disable device voice/beep if needed: self.conn.disable_device()
            logger.info(f"✅ Connected to Biometric Device at {self.device_ip}")
            return True
        except Exception as e:
    ...
    def call_server(self, emp_id):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # FIXED: Accessing the 'data' key instead of 'record'
                attendance_data = result.get('data', {})
                emp_name = attendance_data.get('employee_name', 'Unknown')
                time_val = attendance_data.get('timestamp', '--:--')
                
                logger.info(f"✨ SUCCESS: {emp_name} (ID: {emp_id}) -> {action.upper()} at {time_val}")
                return True
            else:
    ...
    def monitor(self):
        if not self.connect_device(): return
        
        # Initialize count so we don't process old history
        try:
            initial_attendance = self.conn.get_attendance()
            self.last_count = len(initial_attendance)
            logger.info(f"📊 Ready. Ignoring {self.last_count} historical records.")
        except: self.last_count = 0

        logger.info("🔄 Monitoring for new scans...")
        try:
            while True:
                attendance = self.conn.get_attendance()
                if len(attendance) > self.last_count:
                    # New scans found
                    new_records = attendance[self.last_count:]
                    for log in new_records:
...
if __name__ == "__main__":
    monitor = BiometricMonitor(DEVICE_IP, DEVICE_PORT, SERVER_URL)
    monitor.monitor()